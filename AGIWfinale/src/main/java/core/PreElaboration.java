...
public class PreElaboration {
	...
	public PreElaboration(String transfermarkt, String freebaseObjects,
			String SubjectNoObject) throws IOException {
	...
	public void wordCorrector(String output) throws IOException {
		...
		while ((lt = trmkt_reader.readLine()) != null) {
			...
			if (correttaPlay == null) {
				correttaPlay = output
						.equals("Dati_stadi/Stadium_correct_transfermarkt.txt") ? getCorrect(
						parts_trmkt[1], freebaseSubject) : getCorrectPlayer(
						parts_trmkt[1], freebaseSubject);
	...
	public String getCorrect(String toCorrect, String frbs_file)
			throws IOException {
		...
		while ((lf = frbs_reader.readLine()) != null) {
			...
			String toCorrect_sw = sw.clean(toCorrect).toLowerCase()
					.replaceAll("[.|,]", "").replaceAll("-", " ");
			String parts_frbs1_sw = sw.clean(parts_frbs[1]).toLowerCase()
					.replaceAll("[.|,]", "").replaceAll("-", " ");
			float leven = metric.compare(toCorrect_sw, parts_frbs1_sw);
			if (new DifferentContains().contains(toCorrect_sw, parts_frbs1_sw,
					sw) || leven >= 0.8) {
				if (leven > v_max) {
					v_max = leven;
					s_max = parts_frbs[1];
	...
	public String getCorrectPlayer(String toCorrect, String frbs_file)
			throws IOException {
		BufferedReader frbs_reader = new BufferedReader(new FileReader(frbs_file));
		String lf;
		float v_max = 0;
		...
		while ((lf = frbs_reader.readLine()) != null) {
			String parts_frbs[] = lf.split("\t");
			float cosSim = metric.compare(toCorrect.toLowerCase(),
					parts_frbs[1].toLowerCase());
			if (parts_frbs[1].contains(toCorrect)
					|| toCorrect.contains(parts_frbs[1]) || cosSim >= 0.8) {
				if (cosSim > v_max) {
					v_max = cosSim;
					s_max = parts_frbs[1];
				}
